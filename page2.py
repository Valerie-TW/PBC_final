...
for i in range(len(filter2)):
    if filter2[i] == filter3[i] == filter4[i] == filter5[i] == filter6[i] is False:
        filter7.append(True)
    else:
        filter7.append(False)
...
labelTop6 = tk.Label(window, text="選修")
labelTop6.grid(column=0, row=10)
comboExample6 = ttk.Combobox(window, value=a[filter7].values.tolist(), width=50)
